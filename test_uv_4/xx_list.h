...
namespace xx
{
	...
	public:
		...
		size_t Find(T const& v) const noexcept;			// 如果找到就返回索引. 找不到将返回 size_t(-1)
		size_t Find(std::function<bool(T const&)> cond) const noexcept;
		bool Exists(std::function<bool(T const&)> cond) const noexcept;
