...
namespace xx
{
	...
	template<typename T>
	size_t List<T>::Find(T const& v) const noexcept
	{
		for (size_t i = 0; i < dataLen; ++i)
		{
	...
	template<typename T>
	size_t List<T>::Find(std::function<bool(T const&)> cond) const noexcept
	{
		for (size_t i = 0; i < dataLen; ++i)
		{
	...
	template<typename T>
	bool List<T>::Exists(std::function<bool(T const&)> cond) const noexcept
	{
