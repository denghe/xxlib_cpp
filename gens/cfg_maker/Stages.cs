    ...
    public static void Fill(PKG.CatchFish.Configs.Config cfg)
    {
        ...
        // 组合
        {
            var s = NewStage(cfg, 60 * 60);
            {
                ...
                e1.cfg_bornTicksInterval = 2;                           // 两只鱼生成帧间隔
                ...
                // 旋转出鱼--180
                var e2 = CloneElement(e1);
                e2.angle = e2.cfg_angleBegin = (float)System.Math.PI;

                // 圆环鱼阵--小鱼
                var e3 = NewElement<PKG.CatchFish.Stages.Emitter_RingFishs>(s);
                e3.cfg_beginTicks = e1.cfg_endTicks;                    // 生效时间：位于 旋转出鱼 结束之后
                e3.cfg_numFishsPerBatch = 36;                           // 每波产生多少只
                e3.cfg_bornTicksInterval = 60;                          // 波与波之间生成帧间隔
                e3.cfg_speed = 3;
                e3.cfg_scale = 1;
                e3.cfg_coin = 1;

                // 圆环鱼阵--中鱼( 与 圆环鱼阵--小鱼 交叠起作用 )
                var e4 = CloneElement(e3);
                e4.cfg_beginTicks += e3.cfg_bornTicksInterval / 2;      // 生效时间：比 圆环鱼阵--小鱼 晚 波与波之间生成帧间隔 / 2
                e4.cfg_numFishsPerBatch = 18;
                e4.cfg_scale = 2;
                e4.cfg_coin = 2;
            }
            {
