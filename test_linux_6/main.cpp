...
inline int TcpListener::Accept(int const& listenFD) {
	...
	if (fd >= ep->fdHandlers.size()) return fd;
...
inline Epoll::Epoll(int const& maxNumFD) {
	// 创建 epoll fd
	efd = epoll_create1(0);
	if (-1 == efd) throw - 1;
...
inline int Epoll::MakeListenFD(int const& port) {
	char portStr[20];
...
inline int Epoll::Ctl(int const& fd, uint32_t const& flags, int const& op) {
	epoll_event event;
	event.data.fd = fd;
	event.events = flags;
...
inline int Epoll::CloseDel(int const& fd) {
	assert(fd != -1);
	close(fd);
	return epoll_ctl(efd, EPOLL_CTL_DEL, fd, nullptr);
}

inline int Epoll::Wait(int const& timeoutMS) {
...
inline int Epoll::Run(double const& frameRate) {
	assert(frameRate > 0);
...
inline std::shared_ptr<L> Epoll::TcpListen(int const& port, Args&&... args) {
...
inline std::shared_ptr<C> Epoll::TcpDial(char const* const& ip, int const& port, int const& timeoutInterval, Args&&... args) {
...
inline std::shared_ptr<T> Epoll::Delay(int const& interval, std::function<void(Timer* const& timer)>&& cb, Args&&...args) {
